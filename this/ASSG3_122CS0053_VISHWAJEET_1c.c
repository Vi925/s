// Implementation: Predictive Parser (LL(1) Table-Driven - C Implementation) 
...
int nullables[26] = {0};
int **table;
// check if a given variable so that we can remove unnecessary ones
...
// exit(0);
            ...
            if (is_nullable(a[i].der))
            {
                for (int k = 0; k < 255; k++)
                {
                    if (follow[a[i].var - 'A'][k] && table[a[i].var -
                                                           'A'][k] == -1)
                        table[a[i].var - 'A'][k] = i;
                    else if (follow[a[i].var -
                                    'A'][k] &&
                             table[a[i].var - 'A'][k] != -1)
                    {
printf("Conflict while filling the predictive parsing table!\nGrammar is not LL(1)\n");
        ...
        else
        {
            for (int k = 0; k < 255; k++)
            {
                if (follow[a[i].var - 'A'][k] && table[a[i].var -
                                                       'A'][k] == -1)
                    table[a[i].var - 'A'][k] = i;
                else if (follow[a[i].var - 'A'][k] && table[a[i].var -
                                                            'A'][k] != -1)
                {
printf("Conflict while filling the predictive parsing table!\nGrammar is not LL(1)\n");
printf("For variable %c and terminal %c tried to fill %d but found %d\n",a[i].var,k,i,table[a[i].var-'A'][k]);
...
printf("|");
    ...
    word[l + 1] = '\0';
    char stack[500];
    int top = 0, curr = 0, c = 0;
    char sentencial[100];
    sentencial[0] = argv[1][0];
    sentencial[1] = '\0';
    printf("%s --> ", sentencial);
    stack[0] = '$';
...
printf("Error while parsing: no rule found for %c and %c\n",stack[top-1],word[curr]);exit(0);
            ...
            else
            {
                ...
                int len = strlen(a[table[stack[top - 1] -
                                         'A'][word[curr]]]
                                     .der);
                int f = 0;
                for (int i = 0; i < len; i++)
                {
                    if (a[table[stack[top - 1] -
                                'A'][word[curr]]]
                            .der[i] == '@')
                    {
                        // printf(" *** ");
